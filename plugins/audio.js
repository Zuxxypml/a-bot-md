...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  try {
    ...
    if (!/audio/i.test(mime))
      throw `Reply to a voice note/audio you want to modify with caption *${
        usedPrefix + command
      }*`;
    ...
    exec(`ffmpeg -i ${media} ${set} ${filename}`, async (err) => {
      ...
      if (err) return Promise.reject(`_*Error during conversion!*_`);
...
handler.help = [
  ...
  "smooth",
  "tupai",
].map((v) => v + " [reply to voice note/audio]");
