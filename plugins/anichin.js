...
const Detail = async (url) => {
  try {
    ...
    const studio = $('.info-content .spe span:contains("Studio") a')
      .text()
      .trim();
    const released = $('.info-content .spe span:contains("Released")')
      .text()
      .replace("Released:", "")
      .trim();
    const duration = $('.info-content .spe span:contains("Duration")')
      .text()
      .replace("Duration:", "")
      .trim();
    ...
    switch (action.trim().toLowerCase()) {
      case "latest":
        ...
        response = latestResults
          .map(
            ({ title, url, episode, thumbnail, type }) =>
              `*${title}*\nEpisode: ${episode}\nType: ${type}\nURL: ${url}\nThumbnail: ${thumbnail}\n`
          )
          .join("\n");
        break;
      ...
      case "search":
        if (!query) return m.reply("Please provide a search keyword.");
        const searchResults = await Search(query.trim());
        response = searchResults
          .map(
            ({ title, type, status, link, image }) =>
              `*${title}*\nType: ${type}\nStatus: ${status}\nURL: ${link}\nThumbnail: ${image}\n`
          )
          .join("\n");
        break;
      ...
      case "detail":
        if (!query) return m.reply("Please provide the URL for details.");
      ...
      case "download":
        if (!query)
          return m.reply("Please provide the URL for download links.");
        const downloadResults = await Download(query.trim());
        response = downloadResults
          .map(
            ({ resolution, links }) =>
      ...
      default:
        response =
          "Unknown command. Please use: latest, search, detail, episode, download, popular.";
    }
