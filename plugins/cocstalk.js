...
async function scrapeCOC(playerTag) {
  try {
    ...
    const getText = (selector, regex) =>
      $(selector)
        .filter((_, el) => $(el).text().includes(regex.split(" ")[0]))
        .text()
        .match(new RegExp(regex))?.[1] || "Not found";

    return {
      name: $("h2.pt-3").text().trim() || "Not found",
      clan: $("h3.pb-2 a").text().trim() || "No clan",
      level: getText("div.card-body", "Level (\\d+)"),
      trophies: getText("div.card-body", "Trophies (\\d+)"),
      bestTrophy:
        $("div.card-body")
          .filter((_, el) => $(el).text().includes("Best Season"))
          .text()
          .match(/Trophies (\d+)/)?.[1] || "Not found",
...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  if (!args[0]) throw `*Example:* ${usedPrefix}${command} 8000QP00C`;

  try {
    const playerTag = args[0].toUpperCase().replace(/#/g, "");
    const data = await scrapeCOC(playerTag);

    if (!data) throw "*Data not found or an error occurred!*";
