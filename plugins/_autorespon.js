...
${
  banned
    ? "_*Kamu telah di banned/dilarang menggunakan bot!*_\n_Hubungi Owner untuk membuka banned_"
    : `Ada yg bisa dibantu?`
}\n` + teks2,
      m
    );
  ...
  if (/ bot |^bot | bot$/i.test(m.text) && !chats.isBanned) {
    ...
    if (m.chat in this.respon) {
      if (
        this.respon[m.chat].date > new Date() * 1 &&
        this.respon[m.chat].date !== 0
      )
        return;
    ...
    m.reply(
      this.pickRandom([
        ...
        "Apa sob?",
        'Apa panggil" -_-',
        "Bot bot bot bot tross",
        'Kalau mau pakai, pakai aja.. Jngn pnggil" trus..',
        "Piuuuu.... Dummmm... ",
        "Tetetetetetete mantapu jiwaa",
      ]),
      m.chat,
      fakeImgReply
    );
  ...
  if (
    /audio/i.test(m.quoted && m.quoted.mimetype) &&
    m.quoted.seconds == 1 &&
    m.quoted.isBaileys &&
    m.quoted.fromMe
  ) {
  ...
  if (setting.antitag) {
    if (
      m.mentionedJid &&
      m.mentionedJid.includes(
        db.data.settings[this.user.jid].owner || owner[2] + "@s.whatsapp.net"
      ) &&
      !chats.isBanned
    )
      m.reply(`Kenapa ngetag" ownerku _-`);
  }
  /* 
    //////////////
    @respon vn
  ...
  if (!m.isGroup) {
    ...
    if (/[a-z]/i.test(m.text)) {
      ...
      conn.reply(m.chat, or[0].trim() + " \n\n " + or[1].trim(), m, {
        contextInfo: { mentionedJid: conn.parseMention(teks2) },
      });
    }
    ...
    if (
      ((this.user.jid == global.conn.user.jid &&
...
`.trim(),
        m,
        { contextInfo: { mentionedJid: [global.owner[2] + "@s.whatsapp.net"] } }
      );
    }
  }
